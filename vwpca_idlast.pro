;+
; NAME:
;    VWPCA_IDLAST (Renamed from PCA.pro from IDLastro library by VW)
;
; PURPOSE:
...
;
; CALLING SEQUENCE:
;    VWPCA_IDLAST, data, eigenval, eigenvect, percentages, proj_obj, proj_atr, 
...
;      The eigenvalues output by VWPCA_IDLAST may have to be scaled by 1/N_OBJ or
;      1/(N_OBJ-1) to agree with other calculations when /COVAR is set.
;
;      VWPCA_IDLAST uses the non-standard system variables !TEXTOUT and !TEXTUNIT.
;      These can be added to one's session using the procedure ASTROLIB.
;
;      The intrinsic IDL function PCOMP  duplicates most
;      most of the functionality of VWPCA_IDLAST, but uses different conventions and
;      normalizations.   Note the following:
;
;   (1) PCOMP requires a N_ATTRIB x N_OBJ input array; this is the transpose
;         of what VWPCA_IDLAST expects
;   (2) VWPCA_IDLAST uses standardized variables for the correlation matrix:  the input 
...
;   (3) VWPCA_IDLAST (unlike PCOMP) normalizes the eigenvectors by the square root
;         of the eigenvalues.
;   (4) VWPCA_IDLAST returns cumulative percentages; the VARIANCES keyword of PCOMP
...
;
;      IDL> VWPCA_IDLAST, data, /COVAR, t = 'pca.dat'
;
...
;
;      IDL> VWPCA_IDLAST, data, eigenval, eigenvect, /SILENT
;
; PROCEDURES CALLED:
;      TEXTOPEN, TEXTCLOSE
;
...
;      Altered name from PCA to VWPCA_IDLAST, swapped to LAPACK 
;      routines for easier error catching and greater stability V. Wild Nov 2008
;      
;- 
PRO VWPCA_IDLAST, data, eigenval, eigenvect, percentages, proj_obj, proj_atr, $
...
; Dispatch table
 ...
 IF N_PARAMS() EQ 0  THEN BEGIN
  print,'Syntax  - VWPCA_IDLAST, data, [eigenval, eigenvect, percentages, proj_obj, proj_atr,'
...
; Carry out eigenreduction
 la_trired, A, D, E     ; D contains diagonal, E contains off-diagonal
 la_triql, D, E, A,status=status ; D contains the eigen-values, A(*,i) -vectors

if status gt 0 then begin
    print,'VWPCA_IDLAST: la_triql failed'
    return
endif
...
;       Open output file 
        ...
        textopen,'VWPCA_IDLAST', TEXTOUT = textout
        printf,!TEXTUNIT,'VWPCA_IDLAST: ' + systime()
